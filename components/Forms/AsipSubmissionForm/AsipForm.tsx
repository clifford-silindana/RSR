...
function AsipForm({
  ...
  const { loading, getApplication } = useOperator();
  ...
  console.log("asipapplication", application);
  // update application if viewing:

  let response;
  if (fetchExisting == true) {
    useEffect(() => {
      getApplication(application.applicationId)
        .then((data) => {
  ...
  return (
    <div>
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark">
          ...
          <div className="flex flex-row justify-between items-center">
            {isAdministrator?.isAdministrator ? (
              <h4 className="text-xl font-semibold text-black dark:text-white">
                {`Annual Safety Improvement Plans(ASIP): ${refNumber}`}
              </h4>
            ) : (
        ...
        <div className="grid grid-cols-7 gap-5 p-7">
